package com.chaos.util.java.pinyin;
 ...
 * @version: v 1.0
 */
public class HanZiConvertToPinYin {
    ...
    private static final String[] PY_STRING = new String[]{"a", "ai", "an", "ang",
    ...
    private static final HanZiConvertToPinYin CHINESE_SPELLING = new HanZiConvertToPinYin();
    private String resource;

    public static HanZiConvertToPinYin getInstance() {
        return CHINESE_SPELLING;
    }
    ...
    public static String convertToPinYin(String str) {
        return HanZiConvertToPinYin.getInstance().convertAll(str);
    }
     ...
     * @return ASCII 码
     */
    private int getChsAscii(String hanZi) {
        int asc = 0;
        try {
            byte[] bytes = hanZi.getBytes("gb2312");
            if ((bytes.length > UtilMagic.INT_TWO) || (bytes.length <= 0)) {
            ...
            if (bytes.length == UtilMagic.INT_TWO) {
                // 中文字符
     ...
     * @return string
     */
    private String convertSingle(String str) {
        String result = null;
        int ascii = getChsAscii(str);
        if ((ascii > 0) && (ascii < UtilMagic.INT_ONE_HUNDRED_SIXTY)) {
     ...
     * @return string
     */
    public String convertAll(@NonNull String str) {
        ...
        for (int j = 0; j < str.length(); j++) {
            ...
            if ((ascii > 0) && (ascii < 160)) {
                ...
                for (int i = (PY_VALUE.length - 1); i >= 0; i--) {
                    if (PY_VALUE[i] <= ascii) {
                        stringBuilder.append(PY_STRING[i]);
                        break;
                    }
                }
            }
     ...
     */
    @NonNull
    private String getSelling(@NonNull String str) {
        String key, value;
        StringBuilder stringBuilder = new StringBuilder();
        for (int i = 0; i < str.length(); i++) {
            key = str.substring(i, i + 1);
            if (key.getBytes().length == 2) {
                value = convertSingle(key);
                if (null == value) {
                    value = "unknown";
                }
            } else {
