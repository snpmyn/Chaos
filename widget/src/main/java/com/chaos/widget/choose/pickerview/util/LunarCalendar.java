...
public class LunarCalendar {
    ...
    /**
     * 表 1900 到 2099 年间农历年份相关信息（24 位 bit 的 16 进制表示）
     * 1. 前 4 位表该年闰哪月
     * 2. 5 - 17 位表农历年份 13 个月的大小月分布（0 小 / 1 大）
     * 3. 后 7 位表农历年首（正月初一）对应公历日期
     * <p>
     * 以 2014 年数据 0x955ABF 为例：
    ...
    private static final int[] LUNAR_MONTH_DAY = {1887, 0x1694, 0x16aa, 0x4ad5,
     ...
     * @param month        农历月
     * @param monthDay     农历日
     * @param areLeapMonth 该月闰月
     * @return 返农历日期对应公历日期（year0、month1、day2）
     */
    @NonNull
    public static int[] lunarToSolar(int year, int month, int monthDay, boolean areLeapMonth) {
        int dayOffset;
        int leapMonth;
        int i;
        ...
        // 该年有闰月
        if (leapMonth != 0) {
            boolean flag = ((month > leapMonth) || (month == leapMonth && areLeapMonth));
            if (flag) {
    ...
    public static int[] solarToLunar(int year, int month, int monthDay) {
        ...
        int days = LUNAR_MONTH_DAY[index];
    ...
    public static int[] solarToLunarDeprecated(int year, int month, int monthDay) {
        ...
        // offset 减每农历年天数算当天农历第几天
        // iYear 最终结果是农历年份，offset 是当年第几天
        ...
        boolean areLeap = false;
        // 当年天数 offset 逐减每月（农历）天数，得当天是本月第几天
        ...
        lunarDate[3] = (areLeap ? 1 : 0);
        Timber.d(year + " - " + month + " - " + monthDay + " ==> " + lunarDate[0] + " - " + lunarDate[1] + " - " + lunarDate[2] + " - " + lunarDate[3]);
        return lunarDate;
    }

    /**
     * 传回农历 year 年 month 月总天数
     *
     * @param year  算的年
     * @param month 算的月
     * @param leap  当月闰月
    ...
    /**
     * 传回农历 year 年 month 月总天数（共 13 个月包闰月）
     *
     * @param year  算的年
     * @param month 算的月
     * @return 传回农历 year 年 month 月总天数
     */
    ...
    /**
     * 传回农历 year 年闰哪月（1 - 12），没闰传 0
     *
     * @param year 算的年
     * @return 传回农历 year 年闰哪个月（1 - 12），没闰传 0
     */
