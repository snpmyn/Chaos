package com.chaos.widget.textview.align;
...
import com.chaos.util.java.vibrator.VibratorUtils;
        ...
        switch (action) {
            ...
            case ACTION_MOVE:
                if (areShowActionMenu || (mActionMenu.getChildCount() == 0)) {
                    ...
                    boolean areTriggerDistance = (Math.abs(event.getX() - mTouchDownX) < triggerLongPressDistanceThreshold)
                            && (Math.abs(event.getY() - mTouchDownY) < triggerLongPressDistanceThreshold);
                    int[] drawPadding = getDrawPadding(areLeftToRight);
                    boolean areInBound = (event.getX() >= drawPadding[0]) && (event.getX() <= getWidth() - drawPadding[2])
    ...
    private void drawVerticalText(Canvas canvas, float lineSpacingExtra, float charSpacingExtra, boolean isLeftToRight) {
        ...
        for (int j = 0; j < textStrLength; j++) {
            ...
            boolean isCurrentLineFinish = (currentLineOffsetY > getHeight() - drawPadding[3])
                    && (!areUnicodeSymbol(jOfChar) || (areUnicodeSymbol(jOfChar) &&
                    currentLineOffsetY + getCharHeight(jOfChar, textPaint) > getHeight() - drawPadding[3] + getTextSize()));
