package com.chaos.fairy.utils.data;
...
public class CaesarCipherUtils {
    ...
    private static char encrypt(char c, int offsetValue) {
        // 如果字符串中某字符是小写字母
        if (c >= FairyMagic.CHAR_SMALL_A && c <= FairyMagic.CHAR_SMALL_Z) {
            // 移动key%26位
            c += offsetValue % 26;
            if (c < FairyMagic.CHAR_SMALL_A) {
                // 向左超界
                c += 26;
            } else if (c > FairyMagic.CHAR_SMALL_Z) {
                // 向右超界
                c -= 26;
            }
        }
        // 如果字符串中某字符是大写字母
        else if (c >= FairyMagic.CHAR_BIG_A && c <= FairyMagic.CHAR_BIG_Z) {
            // 移动key%26位
            c += offsetValue % 26;
            if (c < FairyMagic.CHAR_BIG_A) {
                // 向左超界
                c += 26;
            } else if (c > FairyMagic.CHAR_BIG_Z) {
                // 向右超界
                c -= 26;
            }
        }
